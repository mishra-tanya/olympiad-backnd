...
class ResultController extends Controller
{
    public function store(Request $request)
    {
        ...
        $cacheKey = "user_result_{$data['userId']}_{$data['classId']}_{$data['goalId']}_{$data['testId']}";
...
//         ]);
   ...
   public function getResultsByUser($userId, $classId, $goalId, $testId)
    {
        $cacheKey = "user_result_{$userId}_{$classId}_{$goalId}_{$testId}";
        ...
        $questionCacheKey = 'questions_set_' . md5(implode('_', $questionIds));
        $questions = Cache::remember($questionCacheKey, now()->addDays(30), function () use ($questionIds) {
            return TestQuestions::whereIn('id', $questionIds)->get([
                'id', 'question', 'option_a', 'option_b', 'option_c', 'option_d', 'correct_answer', 'reason'
            ]);
        });
